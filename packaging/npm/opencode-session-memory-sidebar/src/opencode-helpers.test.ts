...
test("parses ps rows with pid parent pgid and rss", () => {
  const row = parseProcessRow(
    "21658 25671 21658 501 S+ 04:53:32 496480 opencode opencode",
  );
    ...
    sampleProcesses: async () => [
      {
        ...
        command: "opencode",
        args: "opencode",
        cwd: "/Users/guribbong/code/testest",
      },
      {
        pid: 201,
        ppid: 100,
...
test("cleans stale orphan helpers when their cwd matches an Opencode workspace", async () => {
  const killed: number[] = [];
  const summary = await collectOpencodeHelperSummary({
    currentPid: 100,
    currentDirectory: "/Users/guribbong/code/testest",
    sampleProcesses: async () => [
      {
        pid: 100,
        ppid: 1,
        ...
        command: "opencode",
        args: "opencode",
        cwd: "/Users/guribbong/code/cancerbroker",
      },
      {
        pid: 301,
        ppid: 1,
