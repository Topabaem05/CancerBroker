...
`cancerbroker setup`은 다음 파일을 갱신합니다.

- Opencode MCP 설정: `~/.config/opencode/opencode.json`
- CancerBroker guard 설정: `~/.config/cancerbroker/config.toml`

guard config 경로를 바꾸고 싶다면 실행 전에 `CANCERBROKER_CONFIG`를 설정합니다.
...
setup 명령은 실제로 수정한 경로와 생성한 백업 경로를 함께 출력합니다.

### 수동 설정 흐름

interactive wizard를 쓰지 않는 최소 흐름은 다음과 같습니다.

1. 바이너리 설치
2. `cancerbroker setup --non-interactive` 실행
3. `~/.config/opencode/opencode.json` 확인
4. `~/.config/cancerbroker/config.toml` 확인
5. `cancerbroker --config ~/.config/cancerbroker/config.toml status --json`으로 검증

Opencode MCP 항목을 다시 제거하려면:
...
로컬 설치 기준의 일반적인 검증 경로는 다음과 같습니다.
